...
| BACnet-Server (Gen 2) | offiziell, B-SA/B-GW, Port 47808. **Es wird nichts automatisch freigegeben:** Laut TechDoc (ControlType 211–215) hat der BACnet-Server eigene Kindobjekte „Digitaler/Analoger Eingang/Ausgang" (Miniserver-Ausgang = BACnet BI/AI, Miniserver-Eingang = BACnet BO/AO), die wie Virtuelle Ein-/Ausgänge **einzeln angelegt und verdrahtet** werden — derselbe Aufwand wie beim Logger, nur mit Polling statt Push. Config-Klassen: `BACnetDevice` (Attribute laut Stringtabelle `InNum`, `IPv6`, `FdAdr`/`FdP`/`FdL`/`FdM` für Foreign-Device-Registrierung, `Rev`), Kinder `BACnetSensor`/`BACnetAsensor`/`BACnetActor`/`BACnetAactor`. **Versuch 05.09.2026:** ein per Skript eingefügtes `<C Type="BACnetDevice" Pt="47808" InNum="4711">` mit je einem `BACnetActor`/`BACnetAactor`-Kind wurde vom Miniserver geladen, **antwortete aber weder auf Who-Is noch auf ReadProperty** (rohe BACnet/IP-Pakete von der HA-Box, `tcpdump`: null Pakete von Port 47808). Vollständige Attributliste unbekannt → **[OFFEN]**, einmal in Config anlegen und diffen. YABE ließ sich ohne Adminrechte nicht installieren (winget: kein User-Scope-Installer); Ersatz: `bacpypes3` oder rohe Pakete. HA hat keine Core-Integration. |
