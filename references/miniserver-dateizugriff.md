...
**Config-Roundtrip [VERIFIZIERT 05.09.2026]:** Die skripterzeugte Seite „HA UDP" mit 62
`OutputRefLM` und dem Logger überlebt „Aus Miniserver laden" → Bearbeiten → „In Miniserver
speichern" in Config 17.2.8.28 vollständig. Config migriert dabei `V="175"` → `V="178"`, setzt
`WF="18432"` an den Referenzen und `WF="16400"` am Logger und **entfernt `MinimumTime="0"`**
(Standardwert wird nicht geschrieben). Die UDP-Pakete kamen danach unverändert an.
...
| BACnet-Server (Gen 2) | **[VERIFIZIERT 05.09.2026]** offiziell, B-SA/B-GW, Port 47808. **Es wird nichts automatisch freigegeben:** sichtbar sind nur die in Config angelegten BACnet-Objekte (Miniserver-Ausgang = `binary-input`, Miniserver-Eingang = `binary-output`, analog entsprechend) plus `device` und `network-port`. Jeder Wert braucht ein eigenes Objekt samt Verdrahtung — derselbe Aufwand wie beim Logger. Dafür kann BACnet **pushen**: `subscribeCOV` liefert Änderungen 2–3 ms nach dem Ereignis, `writeProperty` schaltet Miniserver-Eingänge. XML-Struktur, Objektliste, Dienste und Messwerte in [bausteine-system-schnittstellen.md](bausteine-system-schnittstellen.md), Abschnitt BACnet; Prüfwerkzeug `scripts/bacnet_probe.py` (YABE-Ersatz ohne Adminrechte). Ein per Skript geratenes `BACnetDevice` ohne die Config-Attribute startete den Server **nicht** — Vorlage aus Config nehmen. HA hat keine Core-Integration. |
